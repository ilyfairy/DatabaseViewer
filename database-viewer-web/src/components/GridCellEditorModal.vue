  ...
  >
    <div v-if="column" class="grid-cell-editor-body">
      <div class="grid-cell-editor-meta">
        <n-tag size="small" :bordered="false">{{ column.type || 'unknown' }}</n-tag>
        <n-tag v-if="column.isPrimaryKey" size="small" type="warning" :bordered="false">主键</n-tag>
        <n-tag v-if="foreignKey" size="small" type="info" :bordered="false">外键</n-tag>
        <n-tag v-if="column.isNullable" size="small" :bordered="false">可空</n-tag>
        <n-tag v-if="column.maxLength" size="small" :bordered="false">max {{ column.maxLength }}</n-tag>
      </div>

      <n-alert v-if="warningText" type="warning" :show-icon="false" class="panel-inline-alert">
        {{ warningText }}
      </n-alert>
      ...
      <div v-if="column.isNullable" class="grid-cell-editor-null-toggle">
        <n-checkbox v-model:checked="setNull" :disabled="saving">将该值设为 NULL</n-checkbox>
      </div>

      <template v-if="isBinary && !setNull">
        <div class="grid-cell-editor-binary-toolbar">
          <n-button size="small" type="primary" :disabled="saving" @click="chooseBinaryFile">选择文件替换</n-button>
          <n-button size="small" tertiary :disabled="saving" @click="clearBinarySelection">清空二进制</n-button>
      ...
      <template v-else-if="isBoolean && !setNull">
        <div class="grid-cell-editor-row">
          ...
          <n-select v-model:value="textValue" :options="booleanOptions" :disabled="saving" class="grid-cell-editor-boolean" />
        </div>
      </template>
      ...
      <div class="grid-cell-editor-actions">
        ...
        <n-button type="primary" :loading="saving" @click="submit">保存修改</n-button>
      </div>
    </div>
  </n-modal>
</template>
