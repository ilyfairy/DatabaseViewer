  ...
  <div class="relation-graph-shell">
    <div class="relation-graph-toolbar">
      <div class="relation-graph-legend">
        <n-tag size="small" :bordered="false" type="success">当前</n-tag>
        <n-tag size="small" :bordered="false" type="info">入向 {{ props.reverseGroups.reduce((count, group) => count + group.rows.length, 0) }}</n-tag>
        <n-tag size="small" :bordered="false" type="warning">出向 {{ props.outgoingRelations.length }}</n-tag>
      </div>
      <div class="relation-graph-toolbar-actions">
        <div class="relation-graph-toolbar-note">拖动画布平移，滚轮缩放；单击选中，双击节点直接跳转。</div>
        <div class="relation-graph-controls">
          <n-button size="small" tertiary @click="zoom = clampZoom(zoom * 0.9)">缩小</n-button>
          <n-button size="small" tertiary @click="zoom = clampZoom(zoom * 1.12)">放大</n-button>
          <n-button size="small" tertiary type="primary" @click="fitToView">重置视图</n-button>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedNode" class="relation-graph-inspector">
      ...
      <div class="relation-graph-inspector-actions">
        <n-tag size="small" :bordered="false" :type="selectedNode.kind === 'current' ? 'success' : selectedNode.kind === 'incoming' ? 'info' : selectedNode.kind === 'outgoing' ? 'warning' : 'default'">
