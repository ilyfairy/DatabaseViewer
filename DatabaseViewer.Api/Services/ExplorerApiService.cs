...
public sealed class ExplorerApiService
{
    ...
    public async Task<TableCellUpdateResponse> UpdateTableCellAsync(TableCellUpdateRequest request)
    {
        ...
        var parsedValue = ConvertUpdateValue(column, request);
    ...
    private static ConnectionDefinition MapCreateConnectionRequest(CreateConnectionRequest request)
    {
        ...
        if (authenticationMode == AuthenticationMode.UsernamePassword && string.IsNullOrWhiteSpace(request.Username))
        {
        ...
        return new ConnectionDefinition
        {
            ...
            Username = authenticationMode == AuthenticationMode.WindowsIntegrated ? string.Empty : (request.Username?.Trim() ?? string.Empty),
            Password = authenticationMode == AuthenticationMode.WindowsIntegrated ? string.Empty : (request.Password ?? string.Empty),
    ...
    private static object? ConvertUpdateValue(ColumnSchema column, TableCellUpdateRequest request)
    {
        if (request.SetNull)
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(request.Base64Value))
            {
            ...
            try
            {
                return Convert.FromBase64String(request.Base64Value);
            }
        ...
        var text = request.TextValue ?? string.Empty;
